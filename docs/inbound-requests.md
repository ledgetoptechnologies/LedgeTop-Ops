...
Apply Delivery migrations `0116_incoming_upload_hardening.sql` and `0198_incoming_upload_owner_notifications.sql` before deploying the dependent Worker. The first adds the resume fingerprint and a rolling-deploy-safe trigger for exactly-once quota release; the second adds the owner-notification outbox without changing existing upload status. Neither migration grants access. `/health` fails closed with HTTP 503 and lists only missing configuration names when the D1/R2/Turnstile/signer/Workflow prerequisites are incomplete. Do not enable or advertise incoming uploads from an unhealthy environment.
