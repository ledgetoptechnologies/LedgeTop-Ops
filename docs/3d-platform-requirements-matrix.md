...
| Server import roots without arbitrary host paths | In progress | Alias-only API, traversal/symlink/special-file tests, and durable exact-content preview/adopt operations are proven; the project-first UI still needs to use preview/confirm instead of the compatibility task-import shortcut |
...
| Provider credentials, endpoints, probe, enable/disable, rotation and admission managed after bootstrap in Viewer UI | In progress | Encrypted credential storage, private DTO, auto-detect-before-save, probe and enable/disable are proven; endpoint/label/admission editing, token rotation/removal, and preset CRUD still need reachable project-first UI |
| Provider metadata/admission edits and scheduled health refresh remain manageable after bootstrap | In progress + live gate | Backend edit/probe/enable CAS and durable health refresh are proven; complete desktop/mobile management and a live provider probe remain evidence |
...
| GCP generic CSV/GeoJSON, canonical meters, map/pixel marking, EXIF proximity without visibility claims | In progress | Backend parser, conversion, adaptive candidate ranking, private image/correspondence routes and immutable attempt snapshots are proven; the loaded project-first workspace still needs the map/image/pixel marking workflow |
...
| Model versions and output lifecycle; archive, recoverable trash, restore and explicit purge | In progress | Backend historical paging, lifecycle, audit and crash/restart fault suite are proven; attempt history, typed purge, failed-mutation retry, and all archive controls still need reachable project-first UI |
...
| Whole-project public share | In progress | Authenticated project grants and per-published-task staff links exist; a public project-scoped aggregate link has not yet been implemented and must not be implied by the UI |
...
| Dedicated Viewer administration: Dashboard, project/task workflow, sharing, Providers, Diagnostics | In progress + live gate | Project-first catalog/search, shared staff visibility, task metrics/log tail, imports, review/share, provider auto-detection and Diagnostics exist; GCP marking, history/lifecycle/provider edit gaps and executable desktop/390/320 workflows remain |
