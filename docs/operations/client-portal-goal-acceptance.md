...
| Requirement | Current evidence | State | Remaining acceptance |
| --- | --- | --- | --- |
| Organization/project contact roles | Exact-source project `project_contact`/`site_contact` backend with live root/contact checks, immutable revisions, and populated-D1 tests | Verified locally, backend only | Add the reviewed staff route and project-workspace UI. Organization `primary_operational`/`delivery` roles remain missing. Never infer roles or portal access. |
| Project memory and field notes | Versioned fixed-section operational memory backend with optimistic concurrency, idempotency, immutable revisions/audit, post-completion amendment reasons, and reassignment fail-closed tests | Verified locally, backend only | Add the reviewed staff route and project-workspace UI. Attachments require a later exact storage-authority contract. |
| Recurring-project copy-forward | No copy contract or preview | Missing | Copy only explicitly selected operational assignments/memory into an already-created destination project under the same source and canonical customer. Preserve provenance and history; never copy grants, billing, invitations, status, or notification state. |
...
| Project-end expiration plus seven-day grace | Project access terms and deadline latching exist | Verified locally | Add expiry-warning and 24-hour reminder notices, then prove automatic revocation and audit in production. |
| Complete meaningful audit coverage | Client/project timeline, feedback events, delivery/share events, access events in separate stores | Partial | Include delegated-share lifecycle and operational-memory/contact events in the unified timeline. Provide global filtering without exposing secrets or meaningless UI clicks. |
