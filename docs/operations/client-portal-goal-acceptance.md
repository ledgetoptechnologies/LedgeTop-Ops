...
| Project-end expiration plus seven-day grace | Project access terms/deadline latching plus a default-off durable SMTP notice processor for seven-day, 24-hour, and expired collaborator events; exact-authority, starvation, retry, recipient-race, and stable Message-ID tests | Verified locally, default-off | Apply migration 0169, stage with SMTP, then live-prove warnings/expiry without changing membership or independent grants. Inviter notices and a staff notice-history UI remain missing. |
| Complete meaningful audit coverage | Client/project timeline now federates seven exact access ledgers with per-adapter coverage, authoritative source/project joins, encrypted proof-bound pagination, redaction, and refresh-safe access filters | Partial | Publish and live-accept the scoped history. Add project-access notice/expiry audit to the timeline after its authoritative ledger is released, plus operational-memory/contact events and approved global filtering. |
