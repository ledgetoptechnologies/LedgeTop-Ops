...
Legacy per-identity `folder_grant` notices are included as `delivery` items and report `delivery: included_legacy_portal_notices`. Their read/dismiss UPDATE is fenced by live legacy association and project-grant conditions. Release remains blocked until selected-workspace binding and capability revocation are also enforced atomically at that UPDATE; a prior route-level workspace check alone is insufficient.
