...
| Delivery intents | Local `0159` isolates receipts and guest reuse. The August 31 increment adds registry-bound secondary HTTP ingress, source-fenced writes, fair source-aware notification dispatch and migrations `0182`/`0183` plus Operations `0049`. See the increment runbook for verification. | Release the paired schema/code safely. Route existence does not enroll a connector; secondary client/outbound authority and live activation remain separately gated. |
| Native requests and feedback | Local `0184`/`0185` preserve exact source/workspace/identity/project or target ownership; Operations `0050` binds dedicated draft-quote credentials to a connector revision. Storage-only request accounts are never legacy portal accounts. | Follow the migration-first, default-off, per-source release contract. Never fall back to primary, merge colliding IDs, or treat service assignment as authority. |
