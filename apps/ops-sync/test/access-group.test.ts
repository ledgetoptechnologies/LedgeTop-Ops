...
describe("Access rule-group reconciliation", () => {
  ...
  it("replaces include rules with sorted desired emails and preserves constraints", async () => {
    ...
    vi.stubGlobal("fetch",vi.fn(async(url:string,init?:RequestInit)=>{
      calls.push({url,init});
      if(!init?.method)return Response.json({success:true,result:{name:"LTDS Ops Users",exclude:[{geo:{country_code:"XX"}}],require:[]}});
      return Response.json({success:true,result:{name:"LTDS Ops Users"}});
    }));
  ...
  it("reports a failed group update so the event remains retryable", async () => {
    ...
    vi.stubGlobal("fetch",vi.fn(async(_url:string,init?:RequestInit)=>{
      if(!init?.method)return Response.json({success:true,result:{name:"LTDS Ops Users",exclude:[],require:[]}});
