...
function NewServiceRequestWizard({
  ...
  const requestProjectId = fixedScope ? fixedProjectId ?? "" : projectId;
  ...
  const catalogTarget = useRef(requestProjectId);
  const loadedCatalogTarget = useRef<string | null>(null);
  catalogTarget.current = requestProjectId;

  const loadCatalogPage = (cursor: string | null = null, targetProjectId = requestProjectId) => {
    ...
    else { loadedCatalogTarget.current = targetProjectId; setCatalogState("loading"); setCatalog([]); setCatalogCursor(null); setCatalogComplete(false); setCatalogLegacy(false); catalogSource.current = null; }
    setCatalogPageError("");
    loadPortalServiceCatalogPage(targetProjectId || null, cursor, controller.signal).then(result => {
      if (!mounted.current || controller.signal.aborted || sequence !== catalogSequence.current || targetProjectId !== catalogTarget.current) return;
      ...
      loadedCatalogTarget.current = targetProjectId;
  ...
  const reloadCatalog = () => loadCatalogPage(null, requestProjectId);
  ...
  useEffect(() => {
    catalogTarget.current = requestProjectId;
    loadCatalogPage(null, requestProjectId);
    return () => {
      catalogController.current?.abort();
      catalogSequence.current += 1;
    };
  ...
  }), [requestProjectId, title, details, location, preferredStartAt, deliverables, siteContactName, siteContactEmail, siteContactPhone, desiredCompletionAt, points, areaGeoJson, selectedServices, selectedServiceVersions, answers, catalog]);
    ...
    saveChain.current = saveChain.current.catch(() => undefined).then(async () => {
      ...
      try {
        ...
        if (currentLifetime()) {
          ...
          const assignmentsChanged = error.status === 409 && error.body?.code === "service_assignments_changed";
          const servicePolicyChanged = catalogChanged || assignmentsChanged;
          setSaveState(error.status === 409 && !servicePolicyChanged ? "conflict" : "error");
          if (servicePolicyChanged) setDirty(false);
          setMessage(assignmentsChanged
            ? "Service assignments changed for this exact request context. Your selections and answers are preserved; review the currently assigned services before continuing."
            : catalogChanged
          ...
          if (servicePolicyChanged) { reloadCatalog(); goTo("services"); }
        }
        throw caught;
      }
    });
  ...
  const catalogMatchesTarget = loadedCatalogTarget.current === requestProjectId;
  const serviceVersionsCurrent = catalogMatchesTarget && catalogState === "ready" && selectedCatalog.length === selectedServices.length
  ...
  function changeRequestProject(nextProjectId: string) {
    if (nextProjectId === projectId) return;
    catalogTarget.current = nextProjectId;
    changePricingBasis(() => setProjectId(nextProjectId));
    setMessage("Service availability is specific to the selected project. Review your services for this request context before continuing.");
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if ([409, 422].includes(error.status ?? 0) && ["catalog_changed", "service_assignments_changed"].includes(code ?? "")) {
        reloadCatalog();
        goTo("services");
        setMessage(code === "service_assignments_changed"
          ? "Service assignments changed for this exact request context. Your selections and answers are preserved; review the currently assigned services before submitting."
          : "The Project Alpha service library changed. Your selections and answers are preserved; review the current service versions before submitting.");
      }
      ...
      if (!["catalog_changed", "service_assignments_changed"].includes(code ?? "")) setMessage(error.message || "The request could not be submitted.");
  ...
  return <form className="portal-request-form portal-request-wizard" onSubmit={submit}>
    ...
    {step === "services" && <section className="portal-wizard-panel" aria-labelledby="request-services-title"><header><span>Step 1 of 5</span><h3 id="request-services-title">What services do you need?</h3><p>Select between 1 and 10 services from the current Project Alpha service library.</p></header>
      {!fixedProjectId && !fixedScope && <label>Project context<select value={projectId} onChange={event => changeRequestProject(event.target.value)}><option value="">New or one-off service</option>{eligibleProjects.map(project => <option key={project.id} value={project.id}>{project.projectName}</option>)}</select><small>Available services are specific to this exact request context.</small></label>}
      <ServiceLibrary catalog={catalogMatchesTarget ? catalog : []} selected={selectedCatalog} state={catalogMatchesTarget ? catalogState : "loading"} onRetry={reloadCatalog}
...
function WorkspaceTeamPanel({ invitationEmailDelivery, hierarchyScopedInvitations, initialWorkspaceId, workspaceMode = "primary", expectedSourceId = null }: { invitationEmailDelivery: boolean; hierarchyScopedInvitations: boolean; initialWorkspaceId: string | null; workspaceMode?: "primary" | "native"; expectedSourceId?: string | null }) {
  ...
  const clearMutationAmbiguity = () => { pendingInvite.current = null; pendingManager.current = null; setUncertain(false); };
  const clearProtected = () => { clearMutationAmbiguity(); clearProtectedVisuals(); setCanManageMembers(false); setPeerAdminManagement(false); };
  ...
  const refreshAccess = async (id: string) => {
    ...
    setLoading(true); clearProtectedVisuals();
    try {
    ...
    setCanManageMembers(access.canManageMembers); setPeerAdminManagement(workspaceMode !== "native" && access.peerAdminManagement===true); setInviteScopes(access.inviteScopes); setRequestsSupported(workspaceMode !== "native" && access.invitationRequestsSupported);
    setAddressBookAvailable(workspaceMode !== "native" && access.addressBookAvailable === true); setCanManageAddressBook(workspaceMode !== "native" && access.canManageAddressBook === true);
    if (workspaceMode === "native" || access.addressBookAvailable !== true || access.canManageAddressBook !== true) setAddressContact(null);
    ...
    setAccessSource(access.sourceId); setOrganizationWide(false); setWideConfirmed(false); setCanRequest(!fallbackCapabilities.includes("delivery.view") && fallbackCapabilities.includes("request.create"));
  ...
  useEffect(() => {
    ...
    loadPortalWorkspaces(undefined, controller.signal).then(async allValues => {
      if (controller.signal.aborted || generation !== lifetime.current) return;
      const values = allValues.filter(value => workspaceMode === "native"
        ? value.resourceMode === "native" && value.id === initialWorkspaceId
        : value.resourceMode !== "native"); setWorkspaces(values);
  ...
  const invite = async (event: FormEvent) => {
    ...
    if (!reviewedCapabilities.length) {setError("Choose a scope with access you can delegate."); return;}
    ...
    setError(""); setReviewed({
        email: email.trim(),
        expectedInvitationPolicyVersion: policy!.version,
        ...(organizationWide
        ...
        ...(addressContact && addressContact.workspaceId === workspaceId && addressContact.sourceId === accessSource && addressContact.email?.toLocaleLowerCase() === email.trim().toLocaleLowerCase() ? {addressContact: {id: addressContact.id, expectedVersion: addressContact.version}} : {}),
    });
  };
  const sendInvitation = async () => {
    ...
    try {
      const result = await invitePortalWorkspaceMember(operation.workspaceId, operation.input, undefined, {idempotencyKey: operation.key, signal: controller.signal});
      if (controller.signal.aborted || generation !== lifetime.current) return;
  ...
  const removeAccess = async (kind: "member" | "invitation", id: string) => {
    if (mutationBusy.current || uncertain || historyLocked || addressBookLocked || !ready || !canManageMembers) return;
  ...
  const secondaryApprovalUnavailable = secondaryWorkspace && approvalRequired;
  const formDisabled = !ready || policy?.mode === "disabled" || secondaryApprovalUnavailable || !(approvalRequired ? requestsSupported : invitationEmailDelivery) || busy || uncertain || historyLocked || addressBookLocked || reviewedManager !== null;
  ...
  return <div className="portal-team-panel">
    {workspaces.length > 1 && <label>Manage team workspace<select value={workspaceId} onChange={event => void selectWorkspace(event.target.value)} disabled={busy || uncertain || historyLocked || addressBookLocked}>{workspaces.map(workspace => <option key={workspace.id} value={workspace.id}>{workspace.displayName}</option>)}</select></label>}
    {loading && <Loading />}
    {workspaceId && <button className="button-ghost" disabled={busy || loading || historyLocked || addressBookLocked} onClick={() => { setError(""); void refreshAccess(workspaceId); }}>Refresh team access</button>}
    ...
    {canInvite && !secondaryApprovalUnavailable && <form onSubmit={invite} className="portal-team-invite-form">
      ...
      {ready && currentWorkspace?.rootType === "organization" && addressBookAvailable && canManageAddressBook && accessSource && <PortalAddressBookPicker workspaceId={workspaceId} sourceId={accessSource} disabled={formDisabled} selected={addressContact} onSelect={contact => {setAddressContact(contact); setEmail(contact.email); clearReview();}} onClear={() => {setAddressContact(null); clearReview();}} onInvalidated={message => {setAddressContact(null); clearProtected(); setError(message);}} />}
        ...
        <div className="portal-hierarchy-tree" role="radiogroup" aria-label="Client hierarchy">
          {filteredRows.map(({ entry, depth }) => {
            ...
            return <label className={`portal-hierarchy-treeitem${selected ? " is-selected" : ""}`} style={{ paddingInlineStart: `${0.75 + depth * 0.9}rem` }} key={`${entry.type}:${entry.publicId}`}>
              <input type="radio" name="invitation-scope" value={`${entry.type}:${entry.publicId}`} disabled={!selectable} checked={selected} onChange={() => { const capabilities = inviteScopes.find(scope => scope.type === entry.type && scope.publicId === entry.publicId)?.capabilities ?? []; setSelectedScope({ type: entry.type, publicId: entry.publicId }); setCanRequest(!capabilities.includes("delivery.view") && capabilities.includes("request.create")); setWideConfirmed(false); setAccessMode(entry.type === "project" && accessOptions.some(option => option.projectPublicId === entry.publicId && option.projectEndSupported) ? "project_end" : ""); setAccessExpires(""); clearReview(); }} />
      ...
      <p className="portal-copy">{selectedCapabilities.includes("delivery.view") ? "Delivery viewing is included for this scope." : "Delivery viewing is not included for this scope."}</p>
      <label className="portal-check"><input type="checkbox" disabled={formDisabled || !selectedCapabilities.includes("request.create") || !selectedCapabilities.includes("delivery.view")} checked={canRequest} onChange={event => { setCanRequest(event.target.checked); clearReview(); }} /> Allow this person to submit service requests for the selected scope</label>
      {canManageMembers && rootCapabilities.length > 0 && <label className="portal-check portal-wide-access"><input type="checkbox" disabled={formDisabled} checked={organizationWide} onChange={event => { const capabilities = event.target.checked ? rootCapabilities : inviteScopes.find(scope => scope.type === selectedScope?.type && scope.publicId === selectedScope?.publicId)?.capabilities ?? []; setOrganizationWide(event.target.checked); setCanRequest(!capabilities.includes("delivery.view") && capabilities.includes("request.create")); setWideConfirmed(false); clearReview(); }} /> {workspaceWideLabel}</label>}
      {broadConfirmationRequired && <div className="portal-danger-disclosure" role="alert"><strong>Broader access</strong><p>The selected capabilities apply to current and future projects across {organizationWide ? "the workspace" : "this organization"}.</p><label className="portal-check"><input type="checkbox" required disabled={formDisabled} checked={wideConfirmed} onChange={event => { setWideConfirmed(event.target.checked); clearReview(); }} /> I understand and want to grant {organizationWide ? "workspace-wide" : "organization-wide"} access.</label></div>}
      {!reviewed && <button className="button-primary" disabled={formDisabled || (!organizationWide && !selectedScope) || (broadConfirmationRequired && !wideConfirmed) || projectScope && (!accessMode || !termsSupported)}>{approvalRequired ? "Review approval request" : "Review invitation"}</button>}
    ...
    {ready && accessSource && !secondaryWorkspace && <PortalInvitationRequests key={`${workspaceId}:${historyRevision}`} workspaceId={workspaceId} sourceId={accessSource} supported={requestsSupported} locked={busy || uncertain || addressBookLocked} onLock={setHistoryLocked} onInvalidated={message => { clearProtected(); setError(message); }} />}
    ...
    {ready && canManageMembers && !uncertain && <div className="portal-team-lists">
      <section><h3>People</h3>{members.length === 0 && <p className="portal-copy">No workspace members are available.</p>}{members.map(member => {
        const sourceManaged = member.source === "project_alpha";
        return <div className="portal-team-row" key={member.identityId}><span><strong>{member.email ?? "Verified portal user"}</strong><small>{member.manager ? "Administrator" : "Member"} · {member.status}</small><small>{sourceManaged ? "Source-managed member" : "Locally invited collaborator"}</small>{sourceManaged&&<small>Access is managed in this Project Alpha source and cannot be changed here.</small>}</span><div className="actions">{peerAdminManagement&&!sourceManaged&&member.status==="active"&&member.canChangeManager===true&&<button className="button-ghost button-small" disabled={managerActionDisabled||reviewed!==null} onClick={()=>{setReviewed(null);setReviewedManager({member,manager:!member.manager});setMessage("");setError("");}}>{member.manager?"Remove administrator":"Make administrator"}</button>}{!sourceManaged && member.status === "active" && <button className="button-ghost button-small" disabled={busy || uncertain || historyLocked || addressBookLocked || !ready || reviewedManager !== null} onClick={() => void removeAccess("member", member.identityId)}>Suspend</button>}</div></div>;
      })}</section>
