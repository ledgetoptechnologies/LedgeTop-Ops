...
export function PortalNotifications({feedbackEnabled,requestsEnabled=true}: {feedbackEnabled:boolean;requestsEnabled?:boolean;nativeWorkspaceId?:string|null}){
  ...
  async function mutate(item:PortalNotificationHistoryItem,action:'read'|'dismiss'){const itemKey=`${item.kind}:${item.id}`;if(busy.includes(itemKey))return;setBusy(value=>[...value,itemKey]);
    ...
    }catch{setError('The notification update could not be confirmed. Try again.');}finally{setBusy(value=>value.filter(id=>id!==itemKey));}}
