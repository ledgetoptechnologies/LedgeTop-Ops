...
const schema=z.object({v:z.literal(4),scope:z.string().length(64),asOf:z.string().datetime(),coverage:z.object({requests:ledger,feedback:ledger,nativeDelivery:ledger,authenticatedDelivery:ledger}).strict(),
  water:z.object({requests:z.number().int().nonnegative(),feedback:z.number().int().nonnegative(),nativeDelivery:z.number().int().nonnegative(),authenticatedDelivery:z.number().int().nonnegative()}).strict(),
  after:z.tuple([z.string().datetime(),z.string().min(1).max(180)]),expires:z.number().int().positive()}).strict().superRefine((value,ctx)=>{
    ...
    if(value.coverage.nativeDelivery!=='included'&&value.water.nativeDelivery!==0)ctx.addIssue({code:'custom',path:['water','nativeDelivery'],message:'omitted native delivery watermark must be zero'});
    if(value.coverage.authenticatedDelivery!=='included'&&value.water.authenticatedDelivery!==0)ctx.addIssue({code:'custom',path:['water','authenticatedDelivery'],message:'omitted authenticated delivery watermark must be zero'});
  });
