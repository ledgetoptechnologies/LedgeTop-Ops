...
const schema=z.object({v:z.literal(1),scope:z.string().length(64),asOf:z.string().datetime(),
  water:z.object({requests:z.number().int().nonnegative(),feedback:z.number().int().nonnegative()}).strict(),
  after:z.tuple([z.string().datetime(),z.string().min(1).max(180)]),expires:z.number().int().positive()}).strict();
