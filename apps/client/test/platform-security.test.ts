...
describe("deployed host admission",()=>{
  ...
  it.each(["production","staging"] as const)("makes portal hosts canonical while retaining an explicit legacy compatibility origin in %s",ENVIRONMENT=>{
    ...
    for(const path of ["/api/internal","/api/internal/other","/api/internal/project-alpha/portal-v2"]){
      expect(requestHostAllowed(`https://client.drone.example${path}`,env)).toBe(false);
    }
...
describe("delivery app shell",()=>{
  ...
  it("denies internal APIs on a legacy client origin before routing while retaining public-share compatibility",async()=>{
    const assetFetch=vi.fn(async(input:RequestInfo|URL)=>new Response(new URL(typeof input==="string"?input:input instanceof URL?input:input.url).pathname));
    const env:any={ENVIRONMENT:"production",EXPECTED_HOST:"portal.drone.example",PUBLIC_BASE_URL:"https://portal.drone.example",PUBLIC_SHARE_ORIGIN:"https://portal.drone.example",CLIENT_PORTAL_ORIGIN:"https://portal.drone.example",CLIENT_PORTAL_ORIGINS:"https://portal.drone.example,https://portal.technology.example",LEGACY_CLIENT_ORIGINS:"https://client.drone.example",ASSETS:{fetch:assetFetch}};
    const context={waitUntil(){},passThroughOnException(){}} as unknown as ExecutionContext;
    const internal=await deliveryWorker.fetch(new Request("https://client.drone.example/api/internal/project-alpha/portal-v2",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}),env,context);
    expect(internal.status).toBe(404);
    await expect(internal.json()).resolves.toEqual({error:"Not found"});
    expect(assetFetch).not.toHaveBeenCalled();
    const publicShare=await deliveryWorker.fetch(new Request("https://client.drone.example/s/existing-link"),env,context);
