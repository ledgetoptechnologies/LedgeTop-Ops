...
import {legacyNotificationCursor,v2NotificationCursor,v3NotificationCursor} from './helpers/legacy-notification-cursor';
...
describe('notification history cursor',()=>{
  ...
  it('binds encrypted continuations to actor and exact source/workspace/root scope',async()=>{
    ...
    const encoded=await encodeNotificationHistoryCursor(env,actor,{v:4,scope:scopeA,asOf:'2026-09-06T12:00:00.000Z',coverage:{requests:'included',feedback:'included',nativeDelivery:'included',authenticatedDelivery:'included'},water:{requests:9,feedback:4,nativeDelivery:2,authenticatedDelivery:3},after:['2026-09-06T11:00:00.000Z','feedback:n1'],expires:Date.now()+60_000});
  ...
  it('rejects a nonzero watermark for an omitted ledger',async()=>{
    await expect(encodeNotificationHistoryCursor(env,actor,{v:4,scope:'a'.repeat(64),asOf:'2026-09-06T12:00:00.000Z',coverage:{requests:'omitted_feature_disabled',feedback:'included',nativeDelivery:'omitted_schema_unavailable',authenticatedDelivery:'omitted_schema_unavailable'},
      water:{requests:1,feedback:2,nativeDelivery:0,authenticatedDelivery:0},after:['2026-09-06T11:00:00.000Z','feedback:n1'],expires:Date.now()+60_000})).rejects.toThrow(/omitted request watermark/);
  });
  it('rejects the prior cursor contract instead of adding delivery rows to an in-flight page',async()=>{
    ...
    const v3 = await v3NotificationCursor(env.DELIVERY_SESSION_SECRET!, actor);
    await expect(decodeNotificationHistoryCursor(env, actor, v3)).resolves.toBeNull();
  });
  it('rejects a nonzero native-delivery watermark when that ledger is omitted',async()=>{
    await expect(encodeNotificationHistoryCursor(env,actor,{v:4,scope:'b'.repeat(64),asOf:'2026-09-06T12:00:00.000Z',coverage:{requests:'included',feedback:'included',nativeDelivery:'omitted_schema_unavailable',authenticatedDelivery:'omitted_schema_unavailable'},
      water:{requests:1,feedback:2,nativeDelivery:1,authenticatedDelivery:0},after:['2026-09-06T11:00:00.000Z','request:n1'],expires:Date.now()+60_000})).rejects.toThrow(/omitted native delivery watermark/);
  });
  it('rejects a nonzero authenticated-delivery watermark when that ledger is omitted',async()=>{
    await expect(encodeNotificationHistoryCursor(env,actor,{v:4,scope:'c'.repeat(64),asOf:'2026-09-06T12:00:00.000Z',coverage:{requests:'included',feedback:'included',nativeDelivery:'included',authenticatedDelivery:'omitted_feature_disabled'},
      water:{requests:1,feedback:2,nativeDelivery:1,authenticatedDelivery:1},after:['2026-09-06T11:00:00.000Z','authenticated_delivery:n1'],expires:Date.now()+60_000})).rejects.toThrow(/omitted authenticated delivery watermark/);
  });
});
