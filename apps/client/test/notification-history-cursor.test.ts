...
describe('notification history cursor',()=>{
  ...
  it('binds encrypted continuations to actor and exact source/workspace/root scope',async()=>{
    ...
    const encoded=await encodeNotificationHistoryCursor(env,actor,{v:1,scope:scopeA,asOf:'2026-09-06T12:00:00.000Z',coverage:{requests:'included',feedback:'included'},water:{requests:9,feedback:4},after:['2026-09-06T11:00:00.000Z','feedback:n1'],expires:Date.now()+60_000});
  ...
  it('rejects a nonzero watermark for an omitted ledger',async()=>{
    await expect(encodeNotificationHistoryCursor(env,actor,{v:1,scope:'a'.repeat(64),asOf:'2026-09-06T12:00:00.000Z',coverage:{requests:'omitted_feature_disabled',feedback:'included'},
