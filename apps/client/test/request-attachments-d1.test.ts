...
describe("request attachment D1 and direct-R2 lifecycle", () => {
  ...
  beforeAll(async () => {
    ...
    await db.batch([
      db.prepare("INSERT INTO client_accounts(id,display_name,status) VALUES ('account-a','Acme','active')"),
      db.prepare("INSERT INTO client_accounts(id,display_name,status) VALUES ('account-b','Other','active')"),
      ...
      ...["draft-a", "draft-removal"].map(id => db.prepare(`INSERT INTO client_service_request_drafts
        (id,account_id,created_by_identity_id,draft_json,create_idempotency_key,create_fingerprint,last_mutation_key)
        VALUES (?,'account-a','identity-a','{}',?,?,?)`).bind(id, `attachment-create-${id}`, "f".repeat(43), `attachment-create-${id}`)),
    ]);
    const bucket = {
  ...
  it("authorizes the draft, sends no source body through Worker, quarantines, scans, and immutably links", async () => {
    ...
    await expect(db.prepare("UPDATE client_service_request_drafts SET state='submitted' WHERE id='draft-a'").run()).rejects.toThrow(/attachments must be accepted or removed/);
    ...
    await db.prepare(`INSERT INTO client_service_requests
      (id,account_id,created_by_identity_id,request_type,title,details,idempotency_key,request_fingerprint)
      VALUES ('request-a','account-a','identity-a','service','Attachment request','Attachment lifecycle fixture',
        'attachment-request-submit',?)`).bind("f".repeat(43)).run();
