...
describe("paged service catalog client boundary", () => {
  it("encodes an opaque continuation and keeps the cancellation signal", async () => {
    ...
    const controller = new AbortController();
    await expect(loadPortalServiceCatalogPage("cursor+/=", controller.signal, request)).resolves.toEqual(page);
    expect(request).toHaveBeenCalledWith("/api/client/service-catalog/page?cursor=cursor%2B%2F%3D", { signal: controller.signal });
  });

  it("uses the legacy endpoint only for initial typed catalog-not-ready responses", async () => {
    const controller = new AbortController();
    const request = vi.fn(async (path: string) => {
      if (path === "/api/client/service-catalog/page") throw { status: 503, body: { code: "catalog_not_ready" } };
      return { services: [] };
    }) as PortalRequest;
    await expect(loadPortalServiceCatalogPage(null, controller.signal, request)).resolves.toEqual({
      services: [], nextCursor: null, complete: false, source: null, legacy: true,
    });
    expect(request).toHaveBeenNthCalledWith(2, "/api/client/service-catalog", { signal: controller.signal });
  });
  ...
  it.each([
    ...
    await expect(loadPortalServiceCatalogPage(null, undefined, request)).rejects.toBe(failure);
  ...
  it("does not mix legacy results into an already-started projected catalog", async () => {
    ...
    const request = vi.fn(async () => { throw failure; }) as PortalRequest;
    await expect(loadPortalServiceCatalogPage("old-page", undefined, request)).rejects.toBe(failure);
    expect(request).toHaveBeenCalledTimes(1);
  });
  ...
  it.each([
    ...
    await expect(loadPortalServiceCatalogPage(null, undefined, respond({ ...page, ...change }))).rejects.toThrow("could not be verified");
  });
});
