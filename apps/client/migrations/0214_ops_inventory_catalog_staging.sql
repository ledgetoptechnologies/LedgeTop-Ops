...
CREATE TABLE ops_inventory_catalog_staging_sources (
  ...
  UNIQUE(source_id,source_instance_id,application_id,history_epoch),
  UNIQUE(registry_id,source_id)
);

CREATE TABLE ops_inventory_catalog_staging_pages (
  registry_id INTEGER NOT NULL REFERENCES ops_inventory_catalog_staging_sources(registry_id) ON DELETE RESTRICT,
  source_id TEXT NOT NULL,
  ...
  PRIMARY KEY(registry_id,snapshot_id,page_index),
  UNIQUE(registry_id,request_id),
  FOREIGN KEY(registry_id,source_id)
    REFERENCES ops_inventory_catalog_staging_sources(registry_id,source_id) ON DELETE RESTRICT
);
...
CREATE TABLE ops_inventory_catalog_staging_items (
  ...
  received_at TEXT NOT NULL DEFAULT(datetime('now')),
  PRIMARY KEY(registry_id,snapshot_id,public_id),
  FOREIGN KEY(registry_id,snapshot_id,page_index)
    REFERENCES ops_inventory_catalog_staging_pages(registry_id,snapshot_id,page_index) ON DELETE CASCADE
);

CREATE INDEX idx_ops_inventory_catalog_staging_items_page
  ON ops_inventory_catalog_staging_items(registry_id,snapshot_id,page_index,public_id);
