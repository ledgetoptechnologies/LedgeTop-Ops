...
-- 0082's existing_mapping dependency is intentionally tied to a legacy
-- project_alpha_directory_mappings row and its acknowledged outbox command.
-- Activated existing bindings have neither row: their durable proof is the
-- activation receipt exposed by project_alpha_active_directory_mappings. Add
-- a separate evidence kind rather than fabricating legacy mapping/outbox
-- history or weakening the legacy evidence contract.
DROP TRIGGER operations_directory_intent_relationship_dependencies_insert_guard;
DROP TRIGGER operations_directory_intent_relationship_dependencies_no_update;
DROP TRIGGER operations_directory_intent_relationship_dependencies_no_delete;
DROP TRIGGER operations_directory_write_fences_exhausted;
DROP TRIGGER operations_directory_materializations_relationship_guard;
DROP TRIGGER operations_directory_materializations_reserve;
DROP TRIGGER client_onboarding_decisions_insert_guard;
DROP VIEW operations_directory_intent_relationship_resolved;
DROP INDEX operations_directory_intent_relationship_parent;

-- Build beside the old table, then replace it by name.  This avoids SQLite's
-- ALTER TABLE RENAME rewrite of the 0082 materialization/write-fence trigger
-- definitions, which are intentionally recreated nowhere in this migration.
CREATE TABLE operations_directory_intent_relationship_dependencies_next (
  ...
  evidence_kind TEXT NOT NULL CHECK(evidence_kind IN ('unlinked','parent_intent','existing_mapping','acquired_mapping')),
  ...
  parent_activation_id TEXT REFERENCES project_alpha_existing_directory_binding_activation_receipts(activation_id) ON DELETE RESTRICT,
  ...
  CHECK((evidence_kind='unlinked' AND organization_record_id IS NULL AND parent_intent_id IS NULL
      AND parent_mapping_command_id IS NULL AND parent_activation_id IS NULL AND parent_public_id IS NULL
      AND parent_ack_revision IS NULL AND parent_ack_command_json IS NULL AND parent_ack_outcome_json IS NULL)
    ...
    OR (evidence_kind='existing_mapping' AND organization_record_id IS NOT NULL AND parent_intent_id IS NULL
      AND parent_mapping_command_id IS NOT NULL AND parent_activation_id IS NULL AND parent_public_id IS NOT NULL
      AND parent_ack_revision IS NOT NULL AND parent_ack_command_json IS NOT NULL AND parent_ack_outcome_json IS NOT NULL)
    OR (evidence_kind='acquired_mapping' AND organization_record_id IS NOT NULL AND parent_intent_id IS NULL
      AND parent_mapping_command_id IS NULL AND parent_activation_id IS NOT NULL AND parent_public_id IS NOT NULL
      AND parent_ack_revision IS NOT NULL AND parent_ack_command_json IS NULL AND parent_ack_outcome_json IS NULL))
);

INSERT INTO operations_directory_intent_relationship_dependencies_next(
  ...
  parent_mapping_command_id,parent_activation_id,parent_public_id,parent_ack_revision,parent_ack_command_json,
  parent_ack_outcome_json,created_at)
SELECT intent_id,client_record_id,client_record_version,relationship_version,relationship_mutation_id,
  organization_record_id,organization_record_version,source_id,source_instance_uuid,application_uuid,
  history_epoch_id,destination_origin,parent_external_canonical_id,evidence_kind,parent_intent_id,
  parent_mapping_command_id,NULL,parent_public_id,parent_ack_revision,parent_ack_command_json,
  parent_ack_outcome_json,created_at
FROM operations_directory_intent_relationship_dependencies;
DROP TABLE operations_directory_intent_relationship_dependencies;
ALTER TABLE operations_directory_intent_relationship_dependencies_next
  RENAME TO operations_directory_intent_relationship_dependencies;
...
-- Recreate the three 0082 dependents after the table swap. They are dropped
-- before the swap below so SQLite cannot retain a reference to a transient
-- table name; their behavior is otherwise unchanged.
...
-- 0083 also validates onboarding decisions against the relationship dependency
-- table. Recreate that guard after the table swap so SQLite does not retain a
-- reference to the removed table object.
