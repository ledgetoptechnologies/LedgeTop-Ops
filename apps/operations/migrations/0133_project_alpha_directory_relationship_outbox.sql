...
SELECT command.command_id,command.expected_current_organization_record_id,'organization',
  history.previous_organization_record_version,command.expected_current_organization_public_id
FROM project_alpha_directory_relationship_outbox command
JOIN operations_directory_client_organization_history history
  ON history.client_record_id=command.client_record_id AND history.relationship_version=command.relationship_version
...
SELECT command.command_id,command.organization_record_id,'organization',
  history.organization_record_version,command.organization_public_id
FROM project_alpha_directory_relationship_outbox command
JOIN operations_directory_client_organization_history history
  ON history.client_record_id=command.client_record_id AND history.relationship_version=command.relationship_version
...
WHERE command.expected_current_organization_record_id IS history.previous_organization_record_id
  ...
  AND NOT EXISTS(SELECT 1 FROM project_alpha_directory_relationship_outbox predecessor
    WHERE predecessor.command_id<>command.command_id AND predecessor.client_record_id=command.client_record_id
      ...
      AND predecessor.relationship_version<command.relationship_version AND predecessor.state<>'acknowledged')
  AND (
    ...
    OR EXISTS(SELECT 1 FROM project_alpha_directory_relationship_outbox evidence
      WHERE evidence.command_id<>command.command_id AND evidence.source_id=command.source_id
        ...
        AND evidence.history_epoch_id=command.history_epoch_id AND evidence.state='acknowledged'
        AND json_extract(evidence.outcome_json,'$.response.result.authorizationGeneration')=command.expected_authorization_generation)
  )
  AND NOT EXISTS(
    SELECT generation FROM (
      ...
      AND (length(generation)>length(command.expected_authorization_generation)
  ...
  AND (
    EXISTS(SELECT 1 FROM project_alpha_directory_outbox evidence
      WHERE evidence.source_id=command.source_id AND evidence.expected_source_instance_id=command.source_instance_id
        AND evidence.application_id=command.application_id AND evidence.expected_history_epoch_id=command.history_epoch_id
        AND evidence.resource_type='client' AND evidence.external_id=command.client_record_id
        AND evidence.state='acknowledged'
        AND json_extract(evidence.outcome_json,'$.response.result.resource.publicId')=command.client_public_id
        AND json_extract(evidence.outcome_json,'$.response.result.resource.revision')=command.expected_client_revision)
    OR EXISTS(SELECT 1 FROM project_alpha_existing_directory_binding_revision_refresh_receipts evidence
      WHERE evidence.record_id=command.client_record_id AND evidence.source_id=command.source_id
        AND evidence.source_instance_id=command.source_instance_id AND evidence.application_id=command.application_id
        AND evidence.history_epoch_id=command.history_epoch_id AND evidence.resource_type='client'
        AND evidence.project_alpha_public_id=command.client_public_id AND evidence.live_revision=command.expected_client_revision)
    OR EXISTS(SELECT 1 FROM project_alpha_existing_directory_binding_activation_receipts evidence
      WHERE evidence.record_id=command.client_record_id AND evidence.source_id=command.source_id
        AND evidence.source_instance_id=command.source_instance_id AND evidence.application_id=command.application_id
        AND evidence.history_epoch_id=command.history_epoch_id AND evidence.resource_type='client'
        AND evidence.project_alpha_public_id=command.client_public_id AND evidence.project_alpha_revision=command.expected_client_revision)
    OR EXISTS(SELECT 1 FROM project_alpha_directory_relationship_outbox evidence
      WHERE evidence.command_id<>command.command_id AND evidence.client_record_id=command.client_record_id
        AND evidence.source_id=command.source_id AND evidence.source_instance_id=command.source_instance_id
        AND evidence.application_id=command.application_id AND evidence.history_epoch_id=command.history_epoch_id
        AND evidence.state='acknowledged' AND evidence.relationship_version<command.relationship_version
        AND json_extract(evidence.outcome_json,'$.response.result.client.publicId')=command.client_public_id
        AND json_extract(evidence.outcome_json,'$.response.result.client.revision')=command.expected_client_revision)
  )
  AND (command.organization_record_id IS NULL OR (
    EXISTS(SELECT 1 FROM project_alpha_directory_outbox evidence
      WHERE evidence.source_id=command.source_id AND evidence.expected_source_instance_id=command.source_instance_id
        AND evidence.application_id=command.application_id AND evidence.expected_history_epoch_id=command.history_epoch_id
        AND evidence.resource_type='organization' AND evidence.external_id=command.organization_record_id
        AND evidence.state='acknowledged'
        AND json_extract(evidence.outcome_json,'$.response.result.resource.publicId')=command.organization_public_id
        AND json_extract(evidence.outcome_json,'$.response.result.resource.revision')=command.expected_organization_revision)
    OR EXISTS(SELECT 1 FROM project_alpha_existing_directory_binding_revision_refresh_receipts evidence
      WHERE evidence.record_id=command.organization_record_id AND evidence.source_id=command.source_id
        AND evidence.source_instance_id=command.source_instance_id AND evidence.application_id=command.application_id
        AND evidence.history_epoch_id=command.history_epoch_id AND evidence.resource_type='organization'
        AND evidence.project_alpha_public_id=command.organization_public_id AND evidence.live_revision=command.expected_organization_revision)
    OR EXISTS(SELECT 1 FROM project_alpha_existing_directory_binding_activation_receipts evidence
      WHERE evidence.record_id=command.organization_record_id AND evidence.source_id=command.source_id
        AND evidence.source_instance_id=command.source_instance_id AND evidence.application_id=command.application_id
        AND evidence.history_epoch_id=command.history_epoch_id AND evidence.resource_type='organization'
        AND evidence.project_alpha_public_id=command.organization_public_id AND evidence.project_alpha_revision=command.expected_organization_revision)
  ));
