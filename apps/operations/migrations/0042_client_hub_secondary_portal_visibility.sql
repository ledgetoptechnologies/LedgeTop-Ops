-- Client Hub is a rebuildable display cache, never an authorization source.
-- Preserve every cached row while allowing an independently authorized
-- secondary business root to remember its exact native portal workspace.
...
-- Reconcile live authority into the cache after deploy. Existing rows remain
-- available during the bounded pass; no portal membership or grant is created.
