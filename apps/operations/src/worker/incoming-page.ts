...
export function incomingRequestPage(input: {
  ...
  const turnstile = siteKey
    ? `<div class="cf-turnstile" data-sitekey="${siteKey}" data-action="incoming-upload" data-size="compact"></div>`
    : "";
  return `<!doctype html>
...
.drop{display:flex;width:100%;min-height:184px;flex-direction:column;align-items:center;justify-content:center;gap:10px;border:2px dashed #9cb4cc;border-radius:14px;padding:30px;text-align:center;background:#f8fafc;cursor:pointer}.drop.drag{border-color:#f05a14;background:#fff6f1}.drop strong{overflow-wrap:anywhere}.drop-help{max-width:42rem;color:#5b6878;line-height:1.5;font-weight:400}.drop input{border:0;padding:0}
...
.honeypot{position:absolute!important;left:-10000px!important;width:1px!important;height:1px!important;overflow:hidden!important}.file-head{display:flex;align-items:center;justify-content:space-between;gap:12px}.file-head strong{min-width:0;overflow-wrap:anywhere}.cancel{flex:none;padding:6px 10px;background:#fff;color:#8c2424;border:1px solid #d9a6a6;font-size:.82rem}
...
.notice.warning{background:#fff7df;color:#765409}.drop input{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap}.drop:focus,.drop:focus-within,input:focus-visible,textarea:focus-visible,button:focus-visible{outline:3px solid #2b6cb0;outline-offset:3px}.action-area{display:grid;justify-items:center;gap:12px;margin-top:18px;text-align:center}.cf-turnstile{width:150px;min-height:140px;max-width:100%}.hint{margin:0;font-size:.9rem;color:#64748b}
...
@media(max-width:580px){.grid{grid-template-columns:1fr}.panel{padding:20px}.drop{min-height:164px;padding:24px 16px}.file-head{align-items:flex-start}}
...
<p>Send files securely. Files pass basic checks before server pickup.</p>
...
${input.requiresAccessCode ? '<div class="field"><label for="code">Access code</label><input id="code" type="password" maxlength="128" required autocomplete="off"></div>' : ""}
...
<div class="action-area">${turnstile}<button id="continue" type="button">Continue securely</button><p class="hint">Files are checked before they can be delivered.</p></div></section>
<section id="upload" class="hidden"><label class="drop" id="drop" for="files" tabindex="0" role="button" aria-describedby="drop-help"><strong>Choose files or drop them here</strong><span class="drop-help" id="drop-help">Uploads begin automatically. To resume after reopening this page, select the same files again.</span><input id="files" type="file" multiple></label></section>
...
document.querySelector("#continue").onclick=async(event)=>{const button=event.currentTarget,name=document.querySelector("#name"),email=document.querySelector("#email"),accessCode=document.querySelector("#code");if(!name.reportValidity()||!email.reportValidity()||(accessCode&&!accessCode.reportValidity()))return;button.disabled=true;button.textContent="Checking...";try{const token=document.querySelector('[name="cf-turnstile-response"]')?.value||"";await api("/authorize",{name:name.value.trim(),email:email.value.trim(),message:document.querySelector("#message").value,accessCode:accessCode?.value||"",turnstileToken:token,website:document.querySelector("#website").value});document.querySelector("#identity").classList.add("hidden");document.querySelector("#upload").classList.remove("hidden");notice("Secure upload session ready.","ok")}catch(error){notice(error.message,"error");button.disabled=false;button.textContent="Continue securely"}};
picker.onchange=()=>{const files=[...picker.files];picker.value="";void uploadFiles(files)};drop.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();picker.click()}});["dragenter","dragover"].forEach(name=>drop.addEventListener(name,event=>{event.preventDefault();drop.classList.add("drag")}));["dragleave","drop"].forEach(name=>drop.addEventListener(name,event=>{event.preventDefault();drop.classList.remove("drag")}));drop.addEventListener("drop",event=>{void uploadFiles([...event.dataTransfer.files])});
</script></body></html>`;
}
