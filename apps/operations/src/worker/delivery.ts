...
export async function listDeliveryFolder(env:Env,principal:StaffPrincipal,prefixValue:string,cursor?:string){
  ...
  const items=files.map(object=>{const id=encodeRef(object.key);const kind=mediaKind(object.key);return{id,name:object.key.slice(prefix.length),kind,size:object.size,uploadedAt:object.uploaded.toISOString(),isShared:shared(object.key),previewUrl:["image","video","audio","pdf","text"].includes(kind)?`/api/delivery/items/${id}/preview`:undefined,thumbnailUrl:kind==="image"?`/api/delivery/items/${id}/thumbnail`:undefined,downloadUrl:`/api/delivery/items/${id}/download`,previewStatus:kind==="video"?"processing":undefined};});
  if(env.STREAM_CUSTOMER_CODE){
    ...
    if(videos.length){
      ...
      await Promise.all(rows.map(async(result,index)=>{const row=result.results[0] as {stream_uid?:string;stream_status?:string}|undefined;const item=items[videos[index]!.index]!;if(row?.stream_status!=="ready"||!row.stream_uid){if(row?.stream_status==="error")item.previewStatus="unavailable";return;}const token=await env.STREAM.video(row.stream_uid).generateToken();item.previewUrl=`/api/delivery/items/${encodeRef(videos[index]!.object.key)}/preview`;item.thumbnailUrl=`https://customer-${env.STREAM_CUSTOMER_CODE}.cloudflarestream.com/${token}/thumbnails/thumbnail.jpg?time=1s&height=340`;item.previewStatus="ready";}));
    }
  }
