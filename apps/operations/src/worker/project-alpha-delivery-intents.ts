...
import { assertProjectAlphaConnectorProof, type ProjectAlphaConnectorProof } from "./project-alpha-connectors";
...
/**
 * Revision fence for an enrolled secondary source. The established primary
 * connection is scalar-configured and intentionally has no row in the portal
 * source-authority registry, so private Ops Sync ingress represents it without
 * a delivery-source proof after it has revalidated the primary connector.
 */
...
export async function applyProjectAlphaDeliveryPreflight(env:Env,payload:unknown,auth:DeliveryAuthentication,
  ...
  authorityProof?:DeliverySourceProof){
