...
async function processJob(env: Env, id: string): Promise<void> {
  ...
  try {
    ...
    for (const object of batch) { const relative = object.key.slice(source.length); const result = await copyObject(env, object.key, `${target}${relative}`, job.conflict_policy,{source,target}); if (job.kind === "move" && !result.skipped) await env.DATA_BUCKET.delete(object.key); }
