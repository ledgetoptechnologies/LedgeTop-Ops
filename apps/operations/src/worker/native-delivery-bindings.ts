...
const selectionSchema = z.object({folderRef:z.string().regex(/^[A-Za-z0-9_-]{2,1400}$/),sourceId:source,
  ...
  reasonCode:z.string().trim().min(1).max(80).regex(/^[A-Za-z0-9_. -]+$/),expiresAt:z.string().datetime({offset:true}).nullable()}).strict();
...
interface StoredGrant { id:string;logical_grant_id:string;grant_version:number;workspace_id:string;folder_binding_id:string;
  ...
  authorization_id:string;fingerprint:string }
...
SELECT json_object('workspaceName',w.display_name,'workspaceRoot',COALESCE(w.pa_organization_public_id,w.pa_client_public_id),
 ...
 'connectorRevision',authority.connector_revision,'connectorVersion',authority.connector_version) proof
...
WHERE generation.id=json_extract(v,'$.generationId') AND (${scopeProofSql})=json_extract(v,'$.scopeProof')
 ...
 AND (EXISTS(SELECT 1 FROM current_entitlements WHERE identity_id=identity.id AND capability='workspace.view'
   AND effect='allow' AND scope_type='workspace' AND scope_public_id=w.id) OR EXISTS(${eligiblePortalShellQuery(false,'w.id','membership.identity_id')}))
...
async function deliveryInput(env:Env,operation:NativeDeliveryGrantInput,prefix:string,ops:OpsProof,bindingId:string){
...
async function prepare(env:Env,principal:StaffPrincipal,operation:NativeDeliveryGrantInput,action:'create'|'revoke'|'audit'='create'):Promise<Prepared>{
  ...
  const input=await deliveryInput(env,operation,prefix,ops.value,bindingId);
  ...
  const result={operation,prefix,bindingId,opsInput:ops.input,opsJson:ops.json,ops:ops.value,deliveryInput:input,deliveryJson:json,deliverySql:DELIVERY_PROOF_SQL,delivery:current,
    contextVersion:await digest([operation,ops.json,json])};
...
function deliveryFence(db:Database,id:string,p:Prepared,extra='1',values:(string|number|null)[]=[]){return db.prepare(`INSERT INTO portal_native_staff_write_fences(id,write_guard)
  VALUES(?,CASE WHEN (${p.deliverySql})=? AND (${extra}) THEN 1 ELSE 0 END)
  ON CONFLICT(id) DO UPDATE SET write_guard=excluded.write_guard`).bind(id,p.deliveryInput,p.deliveryJson,...values);}
...
const HISTORY_PROOF_SQL=`WITH input AS(SELECT json(?) v)
 SELECT json_object('grant',g.id,'version',g.grant_version,'workspace',g.workspace_id,'binding',b.id,'prefix',b.r2_prefix,
   ...
   'authorization',publication.authorization_id,'recipientIdentity',recipient.identity_id) proof
...
function view(row:StoredGrant,p:Prepared,canRevoke:boolean):NativeDeliveryGrantView{return {id:row.id,grantId:row.logical_grant_id,version:row.grant_version,
  ...
  status:row.state==='suspended'||row.state==='revoked'?'revoked':row.status==='active'&&row.expires_at&&Date.parse(row.expires_at)<=Date.now()?'expired':row.status,
...
export async function createNativeDeliveryGrant(env:Env,principal:StaffPrincipal,value:unknown,keyValue:string):Promise<{grant:NativeDeliveryGrantView;replayed:boolean}>{
  ...
  if(row?.state==='active') {await assertOps(env,p);await assertDelivery(env,p);
    if(row.status!=='active'||(row.expires_at&&Date.parse(row.expires_at)<=Date.now()))return changed();
  ...
  try {
    if(!row) {
      ...
      statements.push(db.prepare(`INSERT INTO portal_v2_authenticated_delivery_grants(id,logical_grant_id,grant_version,workspace_id,folder_binding_id,binding_source_version,
        audience_type,audience_public_id,audience_source_version,reason_code,expires_at,created_by_staff_id) VALUES(?,?,1,?,?,?,'principal',?,?,?,?,?)`)
        .bind(auth.grant_id,auth.grant_id,operation.workspaceId,auth.binding_id,p.delivery.ownerVersion,operation.principalPublicId,p.delivery.principalVersion,
          operation.reasonCode,operation.expiresAt,principal.id),
    ...
    if(winner?.state==='active'&&winner.authorization_id===auth.id){
      try {await assertOps(env,p);await assertDelivery(env,p);
        if(winner.status==='active'&&(!winner.expires_at||Date.parse(winner.expires_at)>Date.now()))
...
export async function searchNativeDeliveryTargets(env:Env,principal:StaffPrincipal,folderRef:string,q:string){
  ...
  const targets:Array<{sourceId:string;sourceName:string;workspaceId:string;workspaceName:string;projectId:string;projectName:string}>=[],proofs:Array<{opsInput:string;opsJson:string}>=[];
  for(const candidate of candidates.results.slice(0,100)){let ops;try{ops=await opsProof(env,principal,prefix,candidate.projection_source_id,candidate.id,'create');}
    ...
    const workspaces=await primary(env.DELIVERY_DB).prepare(`SELECT w.id,w.display_name FROM portal_v2_workspaces w
      ...
      .bind(ops.value.connectorRevision,ops.value.projectPublicId,candidate.projection_source_id,ops.value.rootType,ops.value.rootPublicId).all<{id:string;display_name:string}>();
    ...
    targets.push({sourceId:candidate.projection_source_id,sourceName:display(ops.value.sourceName),workspaceId:w.id,workspaceName:display(w.display_name),projectId:candidate.id,projectName:display(ops.value.projectName)});
...
export async function searchNativeDeliveryRecipients(env:Env,principal:StaffPrincipal,value:{folderRef:string;sourceId:string;workspaceId:string;projectId:string;q:string}){
  ...
  for(const row of rows.results.slice(0,50)){try{const p=await prepare(env,principal,parse({folderRef:value.folderRef,sourceId:value.sourceId,workspaceId:value.workspaceId,
    projectId:value.projectId,principalPublicId:row.public_id,reasonCode:'staff_delivery',expiresAt:null}));
