...
type AccessTerms = { kind: "customer" | "collaborator"; mode: "specific_date" | "project_end" | "until_revoked"; expiresAt: string | null };
type Target = { sourceId: string; sourceName: string; workspaceId: string; workspaceName: string; projectId: string; projectName: string; projectEndSupported: boolean };
type Recipient = { principalPublicId: string; displayName: string; email: string | null };
type Grant = Omit<Target, "projectEndSupported"> & { id: string; grantId: string; version: number; principalPublicId: string; recipientName: string; recipientEmail: string | null; status: "active" | "revoked" | "expired"; publicationState: "pending" | "active" | "suspended" | "revoked"; expiresAt: string | null; accessTerms: AccessTerms | null; effectiveAccessExpiresAt: string | null; createdAt: string; canRevoke: boolean };
type Input = { folderRef: string; sourceId: string; workspaceId: string; projectId: string; principalPublicId: string; reasonCode: string; expiresAt: string | null; accessTerms: AccessTerms };
type Preview = { operation: Input; contextVersion: string; sourceName: string; workspaceName: string; projectName: string; recipientName: string; recipientEmail: string | null; folderName: string; expiresAt: string | null; accessTerms: AccessTerms | null; effectiveAccessExpiresAt: string | null; projectEndSupported: boolean };
type Operation = { path: string; body: string; key: string; action: "create" | "revoke"; target: Pick<Target, "sourceId" | "workspaceId" | "projectId">; principalPublicId: string; accessTerms?: AccessTerms; grantId?: string; version?: number; cancelUnpublished?: boolean };
...
const validTargetFields = (value: Omit<Target, "projectEndSupported">) => !!value && [value.sourceId, value.sourceName, value.workspaceId, value.workspaceName, value.projectId, value.projectName].every(string);
const validTarget = (value: Target) => validTargetFields(value) && typeof value.projectEndSupported === "boolean";
const validDate = (value: unknown): value is string => typeof value === "string" && Number.isFinite(Date.parse(value));
const validTerms = (value: AccessTerms | null | undefined): value is AccessTerms => !!value && (value.kind === "customer" || value.kind === "collaborator") && (value.mode === "specific_date" ? value.kind === "collaborator" && validDate(value.expiresAt) : ["project_end", "until_revoked"].includes(value.mode) && value.expiresAt === null && (value.kind === "collaborator" || value.mode === "until_revoked"));
const sameTerms = (left: AccessTerms | null | undefined, right: AccessTerms | null | undefined) => validTerms(left) && validTerms(right) && left.kind === right.kind && left.mode === right.mode && left.expiresAt === right.expiresAt;
const termsLabel = (terms: AccessTerms | null) => !terms ? "Existing access — unclassified" : terms.kind === "customer" ? "Customer — retained project history until revoked" : terms.mode === "project_end" ? "Collaborator — project completion + 7 days" : terms.mode === "specific_date" ? "Collaborator — specific expiry date" : "Collaborator — until revoked";
...
const validGrant = (value: Grant) => validTargetFields(value) && string(value.id) && string(value.grantId) && Number.isSafeInteger(value.version) && value.version > 0 && string(value.principalPublicId) && string(value.recipientName) && optionalString(value.recipientEmail) && ["active", "revoked", "expired"].includes(value.status) && ["pending", "active", "suspended", "revoked"].includes(value.publicationState) && optionalString(value.expiresAt) && (value.accessTerms === null || validTerms(value.accessTerms)) && (value.effectiveAccessExpiresAt === null || validDate(value.effectiveAccessExpiresAt)) && typeof value.createdAt === "string" && typeof value.canRevoke === "boolean";
const targetKey = (value: Pick<Target, "sourceId" | "workspaceId" | "projectId">) => JSON.stringify([value.sourceId, value.workspaceId, value.projectId]);
...
const errorMessage = (caught: unknown) => {
  ...
  if (message === "project_access_terms_unavailable") return "Reviewed access terms are unavailable until the database update is ready. Refresh folder access before trying again.";
  if (message === "project_access_terms_invalid" || message === "native_delivery_expiry_conflict") return "The access duration could not be verified. Choose the recipient role and duration again.";
  if (message === "project_access_expiry_elapsed") return "This access duration has already ended. Review a new duration before granting access.";
...
export function NativeDeliveryGrantPanel({ folder, creationEnabled, onBusyChange }: { folder: { id: string }; creationEnabled: boolean; onBusyChange?: (busy: boolean) => void }) {
  ...
  const [accessKind, setAccessKind] = useState<AccessTerms["kind"] | "">(""), [accessMode, setAccessMode] = useState<AccessTerms["mode"] | "">("");
  ...
  const invalidate = useCallback((caught: unknown): boolean => {
    ...
    setError(caught.status === 401 ? "Your session expired. Sign in again, then refresh folder access."
      ...
      : caught.message === "project_access_completion_unavailable" ? "Verified project completion is no longer available. Refresh folder access and choose a specific date or until revoked."
      : caught.message === "project_access_expiry_elapsed" ? "This access duration has already ended. Refresh folder access and review a new duration."
  ...
  const clearTerms = () => { setAccessKind(""); setAccessMode(""); setExpiresAt(""); };
  const clearRecipient = () => { recipientRead.current?.abort(); setRecipients([]); setRecipient(null); setRecipientQuery(""); setRecipientMessage(""); setRecipientBusy(false); clearTerms(); clearReview(); };
  ...
  const review = async () => {
    ...
    if (!accessKind || !accessMode || accessMode === "project_end" && !target.projectEndSupported) { setError("Choose the recipient's role and an available access duration before reviewing."); return; }
    const expiry = accessMode === "specific_date" && expiresAt ? new Date(expiresAt) : null;
    const reason = reasonCode.trim();
    if (!validReason(reason) || accessMode === "specific_date" && (!expiry || !Number.isFinite(expiry.valueOf()) || expiry.valueOf() <= Date.now())) { setError("Enter a reason using letters, numbers, spaces, dots, underscores, or hyphens, and a valid future expiry date."); return; }
    const input: Input = {folderRef: folder.id, sourceId: target.sourceId, workspaceId: target.workspaceId, projectId: target.projectId, principalPublicId: recipient.principalPublicId, reasonCode: reason, expiresAt: expiry?.toISOString() ?? null, accessTerms: {kind: accessKind, mode: accessMode, expiresAt: expiry?.toISOString() ?? null}};
    ...
    try {
      ...
      if (!value?.operation || Object.entries(input).some(([key, field]) => key !== "accessTerms" && value.operation[key as keyof Input] !== field) || !sameTerms(value.operation.accessTerms, input.accessTerms) || !sameTerms(value.accessTerms, input.accessTerms) || value.expiresAt !== input.expiresAt || value.projectEndSupported !== target.projectEndSupported || !(value.effectiveAccessExpiresAt === null || validDate(value.effectiveAccessExpiresAt)) || input.accessTerms.mode === "specific_date" && value.effectiveAccessExpiresAt !== input.accessTerms.expiresAt || input.accessTerms.mode === "until_revoked" && value.effectiveAccessExpiresAt !== null || typeof value.contextVersion !== "string" || !/^[a-f0-9]{64}$/.test(value.contextVersion) || ![value.sourceName, value.workspaceName, value.projectName, value.recipientName, value.folderName].every(string) || !optionalString(value.recipientEmail)) throw invalidResponse();
      setPreview(value);
  ...
  const execute = async (pending: Operation) => {
    ...
    try {
      ...
      if (controller.signal.aborted || !alive.current || version !== epoch.current) return;
      if (!validGrant(result.grant) || typeof result.replayed !== "boolean" || targetKey(result.grant) !== targetKey(pending.target) || result.grant.principalPublicId !== pending.principalPublicId || pending.action === "create" && !sameTerms(result.grant.accessTerms, pending.accessTerms) || result.grant.status !== (pending.action === "create" ? "active" : "revoked") || result.grant.publicationState !== (pending.action === "create" ? "active" : "revoked") || result.grant.version !== (pending.action === "create" ? 1 : pending.version) || pending.grantId && result.grant.grantId !== pending.grantId) throw invalidResponse();
  ...
  const create = () => {
    ...
    void execute({path: base, body: JSON.stringify({...input, expectedContextVersion: preview.contextVersion}), key: crypto.randomUUID(), action: "create", target, principalPublicId: recipient.principalPublicId, accessTerms: input.accessTerms});
  };
  ...
  return <section className="native-grant-panel" aria-label="Connected workspace folder access">
    ...
    {uncertain && operation.current && <><p>Refreshing only checks current grant records. It does not confirm the earlier operation or create access. You can cancel an unpublished grant after reviewing its exact recipient.</p><button className="button-orange" disabled={busy || historyBusy || operation.current.action === "create" && !creationEnabled} onClick={() => void execute(operation.current!)}>Retry same operation</button></>}
    {!blocked && historyReady && <>
      {!creationEnabled && <div className="notice" role="status">New connected-workspace access is paused. Existing access can still be reviewed and revoked.</div>}
      ...
      {target && <>
        ...
        {recipients.length > 0 && <fieldset disabled={disabled}><legend>Exact recipient</legend>{recipients.map(item => <label className="native-grant-choice" key={item.principalPublicId}><input type="radio" name="native-grant-person" checked={recipient?.principalPublicId === item.principalPublicId} onChange={() => { clearReview(); clearTerms(); setRecipient(item); }} /><span><strong>{item.displayName}</strong><small>{item.email || "No email displayed"}</small></span></label>)}</fieldset>}
      </>}
      <div className="form-grid authenticated-grant-fields"><label>Connected access reason<input value={reasonCode} maxLength={80} disabled={disabled} onChange={event => { setReasonCode(event.target.value); clearReview(); }} /></label><label>Recipient role<select value={accessKind} disabled={disabled || !recipient} onChange={event => { const kind = event.target.value as AccessTerms["kind"] | ""; setAccessKind(kind); setAccessMode(kind === "customer" ? "until_revoked" : kind === "collaborator" && target?.projectEndSupported ? "project_end" : ""); setExpiresAt(""); clearReview(); }}><option value="">Choose customer or collaborator</option><option value="customer">Customer</option><option value="collaborator">Collaborator</option></select></label>
      {accessKind === "collaborator" && <label>Collaborator access duration<select value={accessMode} disabled={disabled} onChange={event => { setAccessMode(event.target.value as AccessTerms["mode"] | ""); setExpiresAt(""); clearReview(); }}><option value="">Choose access duration</option><option value="project_end" disabled={!target?.projectEndSupported}>Project completion + 7 days</option><option value="specific_date">Specific date</option><option value="until_revoked">Until revoked</option></select></label>}
      {accessMode === "specific_date" && <label>Connected access expires<input type="datetime-local" required value={expiresAt} disabled={disabled} onChange={event => { setExpiresAt(event.target.value); clearReview(); }} /></label>}</div>
      {accessKind === "customer" && <p>Customers keep access to this folder's completed project history while this grant remains in place. Access can still be revoked; this does not grant other folders or projects.</p>}
      {accessKind === "collaborator" && !target?.projectEndSupported && <p>Verified project completion is not available for this project. Choose a specific date or until revoked.</p>}
      {accessMode === "project_end" && <p>Access ends seven days after the first verified project completion. Reopening does not renew expired access.</p>}
      {accessKind === "collaborator" && accessMode === "until_revoked" && <p>This collaborator's access will not end automatically when the project completes. It remains until revoked.</p>}
      {!preview && <button className="button-orange" disabled={disabled || !creationEnabled || !recipient || !target || !accessKind || !accessMode} onClick={() => void review()}>Review connected workspace access</button>}
      {preview && <section className="native-grant-review" aria-label="Review connected workspace access"><h4 ref={reviewTitle} tabIndex={-1}>Confirm exact folder access</h4><dl>{[["Folder", preview.folderName], ["Source", preview.sourceName], ["Workspace", preview.workspaceName], ["Project", preview.projectName], ["Recipient", `${preview.recipientName}${preview.recipientEmail ? ` (${preview.recipientEmail})` : ""}`], ["Access terms", termsLabel(preview.accessTerms)], ["Access ends", preview.effectiveAccessExpiresAt ? new Date(preview.effectiveAccessExpiresAt).toLocaleString() : preview.accessTerms?.mode === "project_end" ? "Awaiting verified project completion, then 7 days" : "When revoked"]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>{preview.accessTerms?.mode === "project_end" && <p>Reopening does not renew expired access.</p>}<p>Only this verified person receives this folder grant. It does not merge accounts, grant project administration, or send an email.</p><div className="actions"><button className="button-orange" disabled={disabled || !creationEnabled} onClick={create}>Grant connected workspace access</button><button className="button-ghost" disabled={disabled} onClick={() => setPreview(null)}>Cancel review</button></div></section>}
    </>}
    {historyReady && <section aria-label="Connected workspace grant history"><h4>Current connected workspace grants</h4>{grants.length === 0 ? <p>No connected workspace grants are recorded for this folder.</p> : <div className="authenticated-grant-list">{grants.map(grant => <section className="authenticated-grant-row" key={grant.id}><span><strong>{grant.recipientName}</strong><small>{grant.sourceName} · {grant.workspaceName} · {grant.projectName}</small><small>{termsLabel(grant.accessTerms)}</small><small>{grant.publicationState === "pending" ? "Not published — access is not available" : grant.publicationState === "suspended" ? "Revoked — publication was not confirmed" : grant.status} · {grant.effectiveAccessExpiresAt ? `expires ${new Date(grant.effectiveAccessExpiresAt).toLocaleString()}` : grant.accessTerms?.mode === "project_end" ? "awaiting verified project completion, then 7 days" : grant.expiresAt ? `expires ${new Date(grant.expiresAt).toLocaleString()}` : "no fixed expiry"}</small></span>{grant.canRevoke && <button className="button-danger button-small" disabled={busy || blocked || uncertain && (operation.current?.action !== "create" || grant.publicationState !== "pending")} onClick={() => revoke(grant)}>{grant.publicationState === "pending" ? "Cancel unpublished access" : "Revoke connected access"}<span className="sr-only">: {grant.recipientName}</span></button>}</section>)}</div>}</section>}
  </section>;
}
