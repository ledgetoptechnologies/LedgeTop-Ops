...
export class IncrementalSha256 {
  ...
  update(input: Uint8Array): this {
    ...
    while (offset < input.byteLength) {
      ...
      this.buffered += length; offset += length;
      if (this.buffered === 64) { this.transform(this.buffer); this.buffered = 0; }
    }
    return this;
  }
  ...
  private transform(block: Uint8Array): void {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h! + s1 + choice + ROUND[index]! + words[index]!) >>> 0;
  ...
  digestHex(): string {
    ...
    for (let index = 0; index < 8; index += 1)
      tail[tail.length - 1 - index] = Number((bitLength >> BigInt(index * 8)) & 0xffn);
    for (let offset = 0; offset < tail.length; offset += 64) this.transform(tail.subarray(offset, offset + 64));
    return [...this.state].map(word => word.toString(16).padStart(8, "0")).join("");
  }
}

export async function sha256Blob(blob: Blob, onProgress?: (completed: number) => void): Promise<string> {
  ...
  for (;;) {
    ...
    hash.update(result.value); completed += result.value.byteLength; onProgress?.(completed);
  }
