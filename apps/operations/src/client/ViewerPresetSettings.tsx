...
function OptionEditor({ provider, value, onChange }: { provider: ViewerProviderSummary | undefined; value: Record<string, unknown>; onChange: (value: Record<string, unknown>) => void }): ReactElement {
...
function AdvancedJsonEditor({ value, onApply }: { value: Record<string, unknown>; onApply: (value: Record<string, unknown>) => void }): ReactElement {
  const [draft, setDraft] = useState(() => JSON.stringify(value, null, 2));
  const [error, setError] = useState("");
  useEffect(() => { setDraft(JSON.stringify(value, null, 2)); setError(""); }, [value]);
  return <div><textarea aria-label="Advanced preset JSON overrides" value={draft} onChange={event => setDraft(event.target.value)} rows={5} /><button type="button" className="button-ghost button-small" onClick={() => {
    try {
