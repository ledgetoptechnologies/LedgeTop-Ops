      ...
      : error ? <span className="error">{error}</span>
        : asset ? <button type="button" className="image-location-selection-preview" onClick={open} aria-label="Open selected image">
          {asset.thumbnailUrl && !imageFailed
            ? <img src={asset.thumbnailUrl} alt="Selected mapped image thumbnail" loading="lazy" decoding="async" onError={() => setImageFailed(true)} />
            : <span className="file-type-placeholder"><span className="file-kind" aria-hidden="true">Image</span><small>{asset.thumbnailState === "pending" ? "Thumbnail processing\u2026" : "Thumbnail unavailable"}</small></span>}
...
export function ImageLocationMap({ token, locations, scopeLabel, loadAsset, openAsset }: ImageLocationMapProps) {
  ...
  const [selectedAsset, setSelectedAsset] = useState<ImageLocationMapAsset | null>(null);
  ...
  const selectPoint = useCallback(async (assetRef: string) => {
    if (!loadAsset) return;
    selectedRefState.current = assetRef;
    setSelectedRef(assetRef);
    setSelectedAsset(null);
    setSelectionError("");
    setSelectionLoading(true);
    try {
      const asset = await loadAsset(assetRef);
      if (selectedRefState.current === assetRef) setSelectedAsset(asset);
    } catch (error) {
      if (selectedRefState.current === assetRef) setSelectionError((error as Error).message || "Mapped image is no longer available.");
    } finally {
      if (selectedRefState.current === assetRef) setSelectionLoading(false);
    }
  }, [loadAsset]);
  ...
  const handleDialogKeyDown = (event: ReactKeyboardEvent<HTMLElement>) => {
    if (event.key === "Escape") {
      ...
      if (selectedRefState.current) clearSelection();
