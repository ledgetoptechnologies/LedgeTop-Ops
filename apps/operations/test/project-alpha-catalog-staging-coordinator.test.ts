...
import {stageConfiguredProjectAlphaCatalogSnapshot,type OpsCatalogStagingBinding,type OpsCatalogStagingPage,type ProjectAlphaCatalogStagingEnvironment} from "../src/worker/project-alpha-catalog-staging-coordinator";
...
function harness(enabled="true"){
  ...
  const env={PROJECT_ALPHA_CATALOG_STAGING_COORDINATOR_ENABLED:enabled,PROJECT_ALPHA_API_V2_CONNECTIONS:connections,
    OPS_INVENTORY_CATALOG_STAGING:binding} satisfies ProjectAlphaCatalogStagingEnvironment;
  return{env,pages,binding,stage};
}
...
describe("route-less catalog staging coordinator",()=>{
  it("is default-off before reading a connection or calling Client",async()=>{
    ...
    await expect(stageConfiguredProjectAlphaCatalogSnapshot(env,{sourceId},{readSnapshot})).resolves.toEqual({status:"disabled"});
  ...
  it("rechunks large source pages below the Client RPC byte ceiling",async()=>{
    ...
    await expect(stageConfiguredProjectAlphaCatalogSnapshot(env,{sourceId},{readSnapshot:async()=>snapshot}))
  ...
  it("stops on the first failed receipt and reports only bounded diagnostics",async()=>{
    ...
    const result=await stageConfiguredProjectAlphaCatalogSnapshot(env,{sourceId},{readSnapshot:async()=>complete(1)});
  ...
  it("fails closed for disabled sources, malformed commands, and unavailable bindings",async()=>{
    const {env}=harness();
    await expect(stageConfiguredProjectAlphaCatalogSnapshot({...env,PROJECT_ALPHA_API_V2_CONNECTIONS:JSON.stringify({version:1,instances:{[sourceId]:{...connection,enabled:false}}})},{sourceId}))
      .resolves.toEqual({status:"blocked",reason:"source_disabled"});
    await expect(stageConfiguredProjectAlphaCatalogSnapshot(env,{sourceId:"bad source"})).resolves.toEqual({status:"rejected",reason:"invalid_command"});
    await expect(stageConfiguredProjectAlphaCatalogSnapshot({...env,OPS_INVENTORY_CATALOG_STAGING:undefined},{sourceId}))
