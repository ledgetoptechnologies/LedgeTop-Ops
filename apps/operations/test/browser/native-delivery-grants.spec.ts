...
const targetB = {sourceId: "project-alpha:coastal", sourceName: "Coastal Survey Services", workspaceId: "workspace-b", workspaceName: "Coastal Construction", projectId: "ops-project-b", projectName: "Seawall construction documentation"};
const targetC = {sourceId: "project-alpha:mountain", sourceName: "Mountain Survey Services", workspaceId: "workspace-c", workspaceName: "Mountain Engineering", projectId: "ops-project-c", projectName: "Bridge inspection"};
...
async function mock(page: Page, override?: (route: Route, call: Call) => Promise<unknown> | undefined, enabled = true) {
  ...
  await page.route("**/api/**", async route => {
    ...
    if (call.path === "/api/delivery/authenticated-grants") return route.fulfill({json: {folderBindingId: "binding-primary", grants: []}});
    ...
    if (call.path === "/api/delivery/native-grants" && call.method === "POST") {saved = grant(); return route.fulfill({json: {grant: saved, replayed: false}});}
...
test("same folder panel reviews exact native source/workspace/project/person then creates and revokes access", async ({page}) => {
  ...
  expect(create.body).toEqual({folderRef: "folder-acme", sourceId: targetB.sourceId, workspaceId: targetB.workspaceId, projectId: targetB.projectId, principalPublicId: recipientB.principalPublicId, reasonCode: "client_delivery_access", expiresAt: null, expectedContextVersion: previewContext});
