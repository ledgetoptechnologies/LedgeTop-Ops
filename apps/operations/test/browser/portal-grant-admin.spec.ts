...
function primaryPreview(input: Record<string, any>, context = primaryContext) {return {...context, operation: {...input, accessTerms: input.accessTerms ?? null}, contextVersion: "a".repeat(64), workspaceId: "workspace-acme", workspaceLabel: "Acme Workspace", audienceLabel: "Acme Organization", recipientCount: 0, accessTerms: input.accessTerms ?? null, effectiveAccessExpiresAt: input.expiresAt};}
...
async function mockPrimary(page: Page, override?: (route: Route, call: GrantCall) => Promise<unknown> | undefined) {
  ...
  await page.route("**/api/**", async route => {
    ...
    if (call.path.endsWith("/authenticated-grants/audiences")) return route.fulfill({json: {audiences: [{type: "organization", publicId: "org-acme", displayName: "Acme Organization"}]}});
...
async function openPrimary(page: Page) {await page.goto("/delivery"); await page.getByRole("button", {name: "Actions for Acme"}).click(); await page.getByRole("menuitem", {name: "Share", exact: true}).click(); await page.getByRole("button", {name: "Grant to Client Portal"}).click(); await expect(page.getByRole("button", {name: "Refresh authenticated access"})).toBeEnabled();}
async function selectPrimary(page: Page) {const input = page.getByRole("combobox", {name: /Organization, department, client, project, or person/}); await input.fill("Acme"); await expect(page.getByRole("option", {name: /Acme Organization/})).toBeVisible(); await input.press("ArrowDown"); await page.keyboard.press("Enter");}
...
test("nonadministrators retain ordinary sharing without probing authenticated grants or falling back to legacy grants", async ({page}) => {
  ...
  await expect(page.getByRole("status").filter({hasText: "Administrator access is required to manage authenticated Client Portal grants"})).toBeVisible();
  await expect(page.getByRole("button", {name: "Grant to Client Portal", exact: true})).toHaveCount(0);
