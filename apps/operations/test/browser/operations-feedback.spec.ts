...
test("feedback-specific readiness admits filtered staff without synthesizing unrelated operations permissions", async ({page}) => {
  const calls = await mock(page); await page.goto("/"); await expect(page).toHaveURL(/\/clients\/feedback$/); await expect(queue(page).getByRole("heading", {name: "Feedback queue"})).toBeVisible();
  await expect(page.getByRole("heading", {name: "Feedback queue"})).toBeVisible(); await expect(page.getByRole("tab", {name: "Projects", exact: true})).toHaveCount(0);
...
test("disabled feedback never probes its API even on a direct link", async ({page}) => {
  const calls = await mock(page, undefined, false); await page.goto("/clients/feedback/feedback-one"); await expect(page).toHaveURL(/\/operations\/sops$/); await expect(queue(page)).toHaveCount(0); expect(calls.some(call => call.path.startsWith(endpoint))).toBe(false);
});

test("status and server search preserve account scope through history and refresh", async ({page}) => {
  const calls = await mock(page); await page.goto("/clients/feedback?accountId=account-one"); await queue(page).getByRole("textbox", {name: "Search feedback"}).fill("north edge"); await queue(page).getByRole("button", {name: "Search", exact: true}).click();
...
test("empty authorized scan page still allows continuation without claiming queue empty", async ({page}) => {
  ...
  await page.goto("/clients/feedback"); await expect(queue(page).getByText(/Continue to check more records/)).toBeVisible(); await queue(page).getByRole("button", {name: "Load more feedback"}).click(); await expect(queue(page).getByRole("link", {name: "Church survey north edge.jpg"})).toBeVisible();
});

test("New can move directly to Done with an optional client-visible completion note", async ({page}) => {
  const calls = await mock(page); await page.goto("/clients/feedback/feedback-one?status=new&accountId=account-one"); await queue(page).getByRole("button", {name: "Mark Done", exact: true}).click();
  ...
  expect(calls.find(call => call.method === "POST")?.key).toMatch(/^[a-f0-9-]{36}$/); await expect(queue(page).getByRole("link", {name: "Back to feedback queue"})).toHaveAttribute("href", "/clients/feedback?status=new&accountId=account-one");
...
test("server row actions govern buttons and missing original does not produce a replacement link", async ({page}) => {
  ...
  await page.goto("/clients/feedback/feedback-one"); await expect(queue(page).getByText(/original item is no longer available/)).toBeVisible(); await expect(queue(page).getByRole("button", {name: /^Mark /})).toHaveCount(0); await expect(queue(page).locator('a[href^="/portal"]')).toHaveCount(0);
});
...
test("secondary feedback uses only its source-qualified Client Hub deep link", async ({page}) => {
  ...
  await page.goto("/clients/feedback/native_feedback-one");
...
test("primary feedback uses its canonical Client Hub project link", async ({page}) => {
  const actionPath = "/clients/sources/project-alpha%3Aprimary/business/standalone/legacy-client-1/business-projects/legacy-project-1";
  await mock(page, (route, _url, call) => call.path === `${endpoint}/feedback-one` ? route.fulfill({json: detail({target: {...item().target, actionPath}})}) : undefined);
  await page.goto("/clients/feedback/feedback-one");
  const link = queue(page).getByRole("link", {name: "Open original item"});
  await expect(link).toHaveAttribute("href", actionPath);
  await expect(queue(page).locator('a[href*="/clients/sources/project-alpha%3Aprimary/"]')).toHaveCount(1);
  await expect(queue(page).locator('a[href^="/delivery"], a[href^="/jobs"], a[href^="/portal"]')).toHaveCount(0);
});
...
test("late continuation from a prior filter cannot replace the current queue", async ({page}) => {
  ...
  await page.goto("/clients/feedback"); await queue(page).getByRole("button", {name: "Load more feedback"}).click(); await expect.poll(() => captured).toBe(true); await queue(page).getByRole("button", {name: "Done", exact: true}).click(); await expect(queue(page).getByText("Completed view")).toBeVisible(); release(); await expect(queue(page).getByText("Stale protected row")).toHaveCount(0); expect(calls.filter(call => call.query.has("cursor"))).toHaveLength(1);
});

for (const width of [375, 640, 1280, 3440]) test(`feedback queue and completion form layout ${width}px`, async ({page}, info) => {
  await page.setViewportSize({width, height: 1000}); await mock(page); await page.goto("/clients/feedback"); await expect(queue(page).getByRole("link", {name: "Church survey north edge.jpg"})).toBeVisible(); expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1)).toBe(true);
