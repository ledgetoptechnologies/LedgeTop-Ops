...
test("Operations shows a bounded Viewer overview and opens management on the Viewer domain", async ({ page, context }) => {
  ...
  await context.route("https://viewer.ledgetopdroneservices.com/workspace/**", route =>
    route.fulfill({ contentType: "text/html", body: "<!doctype html><title>LTDS Viewer workspace</title>" }));
  await page.route("**/api/**", async route => {
    ...
    if (url.pathname === "/api/viewer/admin-grant" && request.method() === "POST") return route.fulfill({ status: 201, json: {
      ...
      workspaceUrl: `https://viewer.ledgetopdroneservices.com/workspace/${"g".repeat(43)}`,
  ...
  expect(operationsRequests.filter(value => value === "POST /api/viewer/admin-grant")).toHaveLength(1);
});
