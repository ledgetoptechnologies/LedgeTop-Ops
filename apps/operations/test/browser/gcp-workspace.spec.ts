...
test("ground control workspace is private, explicit about proximity, and usable on desktop and mobile", async ({ page }) => {
  ...
  await page.route("**/api/**", async route => {
    ...
    if (url.hostname === "viewer.ledgetopdroneservices.com") {
      if (url.pathname === "/api/v1/admin-sessions/redeem") return route.fulfill({ json: { accessToken: "a".repeat(43), session: { id: `session-${draftRequests + 1}`, subject: "ops:staff-one", permissions: ["viewer.projects.read","viewer.datasets.read","viewer.processing.read","viewer.processing.write","viewer.providers.read","viewer.gcp.read","viewer.gcp.write"], expiresAt: new Date(Date.now() + 30 * 60_000).toISOString() }, units: { default: "imperial", resolved: "imperial" } } });
      ...
      if (url.pathname === "/api/v1/tasks" && request.method() === "GET") return route.fulfill({ json: { tasks: taskExists ? [task] : [], nextCursor: null } });
      if (url.pathname === "/api/v1/tasks" && request.method() === "POST") {
        draftRequests += 1; const posted = request.postDataJSON() as Record<string, unknown>;
        if (!draftSubmissionId) draftSubmissionId = String(posted.submissionId);
        expect(posted.submissionId).toBe(draftSubmissionId); taskExists = true; eventOrder.push("draft");
        if (draftRequests === 1) return route.abort("connectionreset");
        return route.fulfill({ status: 200, json: { task, replayed: true } });
      }
      if (url.pathname === "/api/v1/tasks/task-one/attempts" && request.method() === "POST") { attemptStarted = request.postDataJSON(); eventOrder.push("attempt"); return route.fulfill({ status: 202, json: { attempt: { id: "attempt-one" } } }); }
      if (url.pathname === "/api/v1/processing/outputs") return route.fulfill({ json: { outputs: [], nextCursor: null, totalCount: 0, totalBytes: 0 } });
      if (url.pathname === "/api/v1/processing/providers") return route.fulfill({ json: { providers: [{ id: "provider-one", displayName: "Cluster", type: "clusterodm", endpoint: "http://nodeodm:3000", enabled: true, admissionLimit: 2, activeAttempts: 0, credential: { configured: true, updatedAt: null }, capabilities: { apiVersion: "1", engine: "ODM", engineVersion: "2.2.3", maxImages: null, maxParallelTasks: null, taskQueueCount: 0, totalMemory: null, availableMemory: null, cpuCores: null, providerType: "clusterodm", testedBaseline: "1.5.5", compatibilityWarning: null, options: [{ name: "dsm", type: "bool", domain: [true,false], help: "Generate DSM", value: false }] }, capabilityFingerprint: "f".repeat(64), lastHealth: "healthy", lastHealthAt: null, createdAt: "2026-08-16T00:00:00Z", updatedAt: "2026-08-16T00:00:00Z" }], nextCursor: null } });
  ...
  await page.getByLabel("Task name").fill("Map flight one");
  await page.getByRole("button", { name: "Create draft for GCP" }).click();
  await expect(page.getByText(/recoverable draft creation/)).toBeVisible();
  await page.reload();
  await page.getByRole("button", { name: "Tasks" }).click();
  await page.getByRole("button", { name: "Resume draft creation" }).click();
  ...
  await page.getByRole("button", { name: "Return to Tasks and start first attempt" }).click();
  const taskArticle = page.getByText("Map flight one", { exact: true }).locator("xpath=ancestor::article");
  await taskArticle.getByText("Start processing").click();
  await taskArticle.getByText("Terrain/DEM").click();
  await taskArticle.getByLabel(/Generate DSM/).selectOption("false");
  await taskArticle.getByRole("button", { name: "Start first attempt" }).click();
  await expect.poll(() => attemptStarted).toMatchObject({ providerId: "provider-one", options: { dsm: false } });
  expect(eventOrder).toEqual(["draft", "draft", "mark", "attempt"]);
