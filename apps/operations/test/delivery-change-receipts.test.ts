...
describe("accepted authenticated delivery changes — migrated real D1", { timeout: 240_000 }, () => {
  ...
  beforeAll(async () => {
    ...
    for (const name of ["0170_authenticated_delivery_change_notifications.sql", "0189_primary_staff_folder_bindings.sql", "0204_delivery_change_receipts.sql", "0209_authenticated_delivery_change_recipient_events.sql"])
    ...
    env = { DELIVERY_DB: db, AUTHENTICATED_DELIVERY_NOTIFICATIONS_ENABLED: "true",
      PROJECT_ALPHA_PORTAL_HMAC_SECRET: "primary-portal-hmac-test-key-material-at-least-thirty-two-bytes" } as Env;
