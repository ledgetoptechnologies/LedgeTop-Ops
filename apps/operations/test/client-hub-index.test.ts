...
async function fixture() {
  ...
  await execute(ops, `
    CREATE TABLE pa_organizations(id TEXT PRIMARY KEY,name TEXT,active INTEGER,payload_json TEXT);
    CREATE TABLE pa_clients(id TEXT PRIMARY KEY,name TEXT,organization_id TEXT,active INTEGER,payload_json TEXT);
    CREATE TABLE pa_projects(id TEXT PRIMARY KEY,name TEXT,organization_id TEXT,client_id TEXT,active INTEGER);
  `);
...
describe("resumable Client Hub index", { timeout: 60_000 }, () => {
  it("backfills more than 500 roots with bounded pages and resumes from its persisted checkpoint", async () => {
    ...
    await ops.prepare(`WITH RECURSIVE sequence(n) AS (VALUES(0) UNION ALL SELECT n+1 FROM sequence WHERE n<619)
      INSERT INTO pa_organizations SELECT printf('%04d',n),'Organization '||printf('%04d',n),1,'{}' FROM sequence`).run();
  ...
  it("keeps colliding source records and principal identities separate; only contact scalars are indexed", async () => {
    ...
    await ops.batch([
      ops.prepare("INSERT INTO pa_organizations VALUES('org','Business',1,'{}')"),
      ops.prepare("INSERT INTO pa_clients VALUES('same','Source customer',NULL,1,?)")
        .bind(JSON.stringify({ email: "source@example.test", phone: "+1 (920) 555-1234", price: 98765, notes: "secret business notes" })),
      ops.prepare("INSERT INTO pa_clients VALUES('contact','Business contact','org',1,'{}')"),
      ops.prepare("INSERT INTO pa_projects VALUES('project','Roof survey','org','contact',1)"),
    ]);
    await delivery.batch([
  ...
  it("unchanged reconciliation preserves revision; changed ownership and removals clear stale fields", async () => {
    const { ops, env } = await fixture();
    await ops.batch([
      ops.prepare("INSERT INTO pa_organizations VALUES('a','A',1,'{}')"),
      ops.prepare("INSERT INTO pa_organizations VALUES('b','B',1,'{}')"),
      ops.prepare("INSERT INTO pa_clients VALUES('contact','Contact','a',1,?)").bind('{"email":"old@example.test"}'),
    ]);
    await finish(env);
  ...
  it("does not infer a business identity from an equal raw portal ID, including inactive sources", async () => {
    const { ops, delivery, env } = await fixture();
    await ops.prepare("INSERT INTO pa_organizations VALUES('inactive','Inactive source',0,'{}')").run();
    await delivery.batch([
      delivery.prepare("INSERT INTO portal_v2_workspaces VALUES('old','organization','inactive',NULL,'Old portal','active',NULL)"),
      delivery.prepare("INSERT INTO portal_v2_workspaces VALUES('only','organization','portal-only',NULL,'Portal only','active',NULL)"),
    ]);
    await finish(env);
  ...
  it("does not count a reassigned legacy account under its old workspace root", async () => {
    const { ops, delivery, env } = await fixture();
    await ops.prepare("INSERT INTO pa_organizations VALUES('old','Old owner',1,'{}'),('new','New owner',1,'{}')").run();
    await delivery.batch([
  ...
  it("folds a portal-only row only after explicit native public-ID proof; preserves colliding business IDs", async () => {
    ...
    await ops.prepare("INSERT INTO pa_organizations VALUES('42','Actual business',1,'{}'),('workspace','Unrelated business',1,'{}')").run();
    await delivery.batch([
      delivery.prepare("INSERT INTO portal_v2_workspaces VALUES('workspace','organization',?,NULL,'Portal only','active',NULL)").bind(publicId),
