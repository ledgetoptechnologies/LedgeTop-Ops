...
describe('real D1 native staff folder binding and exact principal delegation',{timeout:60_000,concurrent:false},()=>{
  beforeAll(async()=>{
    ...
    for(const [db,path,cap] of [[ops,new URL('../migrations/',import.meta.url),'0040'],[delivery,new URL('../../client/migrations/',import.meta.url),'0163']] as const)
  ...
  it('uses real explicit target/principal selections and exact preview echoes',async()=>{
    ...
    expect(targets.targets).toContainEqual({sourceId,sourceName:'Secondary Alpha',workspaceId:f.operation.workspaceId,workspaceName:`Workspace ${counter}`,projectId:f.project,projectName:`Native Project ${counter}`});
  ...
  it('source suspension before the Delivery stage leaves only durable OPS evidence',async()=>{
    const f=await fixture(),body=await input(f),wrapped={...env,DELIVERY_DB:intercept(delivery,sql=>sql.some(s=>s.includes('INSERT INTO portal_v2_folder_bindings')),suspendSource)};
